...
resource "azurerm_service_plan" "this" {
  name                = "${var.prefix}-winfun-service-plan"
  resource_group_name = data.azurerm_resource_group.this.name
  location            = data.azurerm_resource_group.this.location
  os_type             = "Windows"
...
resource "azurerm_user_assigned_identity" "this" {
  ...
  name = "${var.prefix}-winfun-identity"
  tags = var.tags
}

resource "azurerm_windows_function_app" "this" {
  name                = "${var.prefix}-winfun-app"
  resource_group_name = data.azurerm_resource_group.this.name
  location            = data.azurerm_resource_group.this.location
  ...
  dynamic "site_config" {
    ...
    content {
      ...
      application_stack {
        dotnet_version              = lookup(site_config.value, "dotnet_version", "6")
        use_dotnet_isolated_runtime = lookup(site_config.value, "use_dotnet_isolated_runtime", "false")
      }
    }
  }
